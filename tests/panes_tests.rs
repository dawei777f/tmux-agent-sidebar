...
fn test_agents_auto_scroll_keeps_selected_visible() {
    ...
    let _ = render_to_string(&mut state, 28, 8);
...
fn test_agents_auto_scroll_shows_last_selected_pane() {
    ...
    // Use a tight height so the agents area is small enough to overflow.
    let _ = render_to_string(&mut state, 28, 8);
...
fn test_agents_auto_scroll_up_shows_group_header() {
    ...
    insta::assert_snapshot!(render_to_string(&mut state, 28, 8), @r"
     ≡8  ●0  ◎0  ◐0  ○8  ✕0
    ⓘ                        — ▾
    project
      ○ claude
