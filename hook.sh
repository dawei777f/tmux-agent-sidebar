...
# Fallback locations used when this script is executed from a Claude Code
# plugin install (e.g. `${CLAUDE_PLUGIN_ROOT}/hook.sh`). The plugin cache
# never contains the binary, so hop over to the rmux plugin directory.
RMUX_CONFIG_PLUGIN_DIR="$HOME/.config/rmux/plugins/tmux-agent-sidebar"
RMUX_PLUGIN_DIR="$HOME/.rmux/plugins/tmux-agent-sidebar"
...
elif [ -x "$RMUX_CONFIG_PLUGIN_DIR/bin/tmux-agent-sidebar" ]; then
  BIN="$RMUX_CONFIG_PLUGIN_DIR/bin/tmux-agent-sidebar"
elif [ -x "$RMUX_CONFIG_PLUGIN_DIR/target/release/tmux-agent-sidebar" ]; then
  BIN="$RMUX_CONFIG_PLUGIN_DIR/target/release/tmux-agent-sidebar"
elif [ -x "$RMUX_PLUGIN_DIR/bin/tmux-agent-sidebar" ]; then
  BIN="$RMUX_PLUGIN_DIR/bin/tmux-agent-sidebar"
elif [ -x "$RMUX_PLUGIN_DIR/target/release/tmux-agent-sidebar" ]; then
  BIN="$RMUX_PLUGIN_DIR/target/release/tmux-agent-sidebar"
else
  exit 0
fi
exec "$BIN" hook "$@"
